...
def show_cam_on_image(img, mask):
    gray_mask=np.uint8(255 * mask)#把激活图调整为0~255范围内，此时是一张灰度图
    heatmap_uint8 = cv2.applyColorMap(gray_mask, cv2.COLORMAP_JET)#给灰度图上色
    heatmap = np.float32(heatmap_uint8) / 255
    cam = heatmap + np.float32(img)/255
    cam = cam / np.max(cam)
    return np.uint8(255 * cam),heatmap_uint8,gray_mask
